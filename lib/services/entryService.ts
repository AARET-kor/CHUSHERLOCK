...
/** Creates an entry and auto-links it to any existing same-category entries
 * whose title overlaps, so related knowledge stays connected instead of
 * duplicated (see lib/codex/overlap.ts). */
export async function createEntry(input: CreateEntryInput): Promise<CreateEntryResult> {
  ...
  const relatedEntryIds = Array.from(
    new Set([...input.relatedEntryIds, ...overlaps.map((o) => o.id)])
  );
